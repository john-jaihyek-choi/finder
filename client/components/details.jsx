...
export default class Details extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      infoIndex: this.props.restaurant.reviews.length,
      photoIndex: 0,
    };
  }
  ...
  renderHours() {
    ...
    return days.map((day, ind) => {
      let hours;
      if (!this.props.restaurant.hours[0].open.length)
        hours = `${day}: No data`;
      else if (!this.props.restaurant.hours[0].open[ind])
        hours = `${day}: Closed`;
      else
        hours = `${day}: ${this.calculateTime(
          this.props.restaurant.hours[0].open[ind].start
        )} - ${this.calculateTime(
          this.props.restaurant.hours[0].open[ind].end
        )}`;
  ...
  cycleInfo() {
    if (this.state.infoIndex === this.props.restaurant.reviews.length)
      return this.renderHours();
    if (!this.props.restaurant.reviews.length) return;
    const rating = this.props.renderRating(
      this.props.restaurant.reviews[this.state.infoIndex]
    );

    return (
      <div className="col-11 d-flex flex-column align-items-center justify-content-center">
        <div className="mb-2">
          {this.props.restaurant.reviews[this.state.infoIndex].text}
        </div>
        <div className="mb-2">{`- ${
          this.props.restaurant.reviews[this.state.infoIndex].user.name
        }`}</div>
      ...
      >
            ...
            onClick={() =>
              this.setState({
                photoIndex:
                  ...
                  this.props.restaurant.photosUrl.length,
              })
            }
            src={
              this.props.restaurant.photosUrl.length === 0
                ? this.props.restaurant.storeImageUrl
                : this.props.restaurant.photosUrl[this.state.photoIndex]
            }
        ...
        <div className="w-100 h-25 mt-2 d-flex flex-column details-text">
          ...
          <div className="w-100 h-25 text-center text-pink font-weight-bold d-flex align-items-center justify-content-center">
            <div className="w-100">
              {this.props.renderRating(this.props.restaurant)}
            </div>{" "}
            |
            <div className="w-100">
              {this.props.renderPrice(this.props.restaurant)}
            </div>{" "}
            |
            <div className="w-100">
              <i className="fas fa-map-marker-alt mr-2"></i>
              {(this.props.restaurant.distance * 0.000621371).toFixed(1)} mi
            </div>
          </div>
        </div>
        <div
          ...
          onClick={() =>
            this.setState({
              infoIndex:
                ...
                (this.props.restaurant.reviews.length + 1),
            })
          }
        >
          {this.cycleInfo()}
