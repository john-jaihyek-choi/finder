...
export default class LikedReviewedCards extends React.Component {
    ...
    rate(event) {
        this.props.postReview(this.props.restaurant.yelpId, this.props.restaurant.note, this.props.thumbsRate, false)
    }
                ...
                <div className={`d-flex flex-wrap ${this.props.viewState === "likedRestaurants" ? "mt-4" : "mt-2"} mb-4 text-pink`}>
                    <div className={`w-50 text-center ${this.props.viewState === "likedRestaurants" ? "col-7 p-0 pr-0" : "col-6"}`}>
                        {this.props.viewState === "likedRestaurants" 
                            ? rating 
                            : <i data-yelpid={this.props.restaurant.yelpId} onClick={this.rate} id="thumbsUp" className={`fa-2x ${this.props.restaurant.thumbsRate === true ? "fas fa-thumbs-up" : "far fa-thumbs-up"}`}></i>}
                    </div> 
                    {this.props.viewState === "likedRestaurants" ? "|" : ""}
                    <div className={`w-50 ${this.props.viewState === "likedRestaurants" ? "col-3 pr-0 pl-1" : "col-6"}`}>
                        {this.props.viewState === "likedRestaurants" 
                            ? price 
                            : <i data-yelpid={this.props.restaurant.yelpId} onClick={this.rate} id="thumbsDown" className={`fa-2x ${this.props.restaurant.thumbsRate === false ? "fas fa-thumbs-down" : "far fa-thumbs-down"}`}></i>}
                    </div>
                    </div>
